...
export default defineConfig({
  test: {
    // Existing suites use global describe/it/expect (no imports), so enable globals.
    globals: true,
    // All included suites are pure logic/util tests (no DOM needed).
    environment: 'node',
    include: ['src/**/__tests__/**/*.test.{ts,tsx}'],
    exclude: [
      '**/node_modules/**',
      '**/dist/**',
      // trackingService.integration.test.ts is written against the Jest API
      // (jest.fn / jest.Mocked) and predates any runner being wired into this
      // package, so it fails under vitest with "jest is not defined". Converting
      // it to vitest (vi.fn) is out of scope here; exclude it until then.
      '**/trackingService.integration.test.ts',
    ],
  },
  resolve: {
    alias: [
