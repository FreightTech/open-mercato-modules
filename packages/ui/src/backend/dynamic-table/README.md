...
- **Column quick filter** - Excel-style AutoFilter funnel in every column header: search a checkbox
  list of distinct values, apply, clear. Ad-hoc and deliberately **never saved**
...
interface TableUIConfig {
  ...
  searchBarStart?: React.ReactNode; // First item in the search-row actions
  searchBarEnd?: React.ReactNode;   // Content at the right end of the search row
...
> **Removed in 0.13.0.** Nine flags were destructured and never read by any render
> path: `hideFilterButton`, `hideBottomBar`, `toolbarPosition`, `hideColumnsButton`,
> `hideFilterPopover`, `hideSortButton`, `hideGroupButton`, `bottomBarStart`,
> `bottomBarEnd`. They belonged to the popover toolbar, which no longer exists —
> the v2 layout uses the column menu and the Configure View drawer instead.
> Deleting them is source-breaking but behaviour-neutral: nothing read them.
...
Add custom buttons or content to the top bar or the search row:
...
<DynamicTable
  uiConfig={{
    ...
    // Add custom content to the right end of the search row
    searchBarEnd: (
...
## Column quick filter (Excel AutoFilter)

Every filterable column header carries a funnel. Clicking it opens a dropdown anchored to that
header with a search box over a checkbox list of the column's distinct values; applying writes an
`is_any_of` rule for that column.

```tsx
<DynamicTable
  columns={columns}
  data={rows}
  // Values for the tick list. Same prop the Configure View drawer uses — supply it
  // once and both pickers work. Without it the funnel falls back to the distinct
  // values of the LOADED page and says so in the dropdown.
  loadFilterSuggestions={async (field, query) => {
    const res = await apiCall(`/api/my-module/filter-suggestions?field=${field}&query=${query}`)
    return res.result?.items ?? []
  }}
/>
```

The value source degrades in three documented steps:

1. a column that declares `source` uses that enum directly (labels shown, raw values stored);
2. otherwise the module's `loadFilterSuggestions` loader is called with the typed needle, debounced
   and superseded (`useSuggestionFetch` — the one implementation both value pickers share);
3. otherwise the distinct values of the currently loaded rows, labelled as a page sample.

Calculated (`formula__*`) columns get no funnel: they are evaluated in the browser after the page
arrives, so a server-side rule on them would match nothing.

### These filters are never saved

Header filters are **ad-hoc by contract**. They live in `DynamicTable`'s `useState` and reach the
server only as query parameters; nothing writes them to a perspective. This is a product decision
from the INF workshop (A2/D6) — a user who filters ten times a day does not want ten saved views —
and it is pinned by `TC-APP-363`, which applies a header filter, reloads and asserts it is gone.

The UI states it rather than relying on the user to know: the dropdown carries a "Temporary — not
saved with the view" footnote, the filtered header takes a **dashed** accent underline, and the
toolbar's "Clear all filters (n)" pill has a dashed border. Saved filters (Configure View →
perspective) keep the solid treatment.

A filtered column keeps its funnel glyph lit for as long as the rule is live, whichever surface
authored it — that is the "am I actually seeing everything?" guarantee (workshop A13).
