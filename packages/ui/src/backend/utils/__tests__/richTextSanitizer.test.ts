...
describe('richTextSanitizer', () => {
  ...
  it('removes executable content and unsafe attributes', () => {
    // <script> is dropped with content (nonTextTags), onclick/style are stripped,
    // <img> is an allowed tag so it is kept but onerror is stripped and src is preserved.
    expect(
  ...
  it('sanitizes plain text paste when it contains html markup', () => {
    // <img> is an allowed tag: src is kept, onerror is stripped.
    // <a href="javascript:…"> has a disallowed scheme so href is removed.
    expect(
