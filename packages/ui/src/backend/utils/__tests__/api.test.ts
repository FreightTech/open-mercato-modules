...
describe('apiFetch', () => {
  ...
  it('returns the 403 payload — and schedules no navigation — when the forbidden redirect is disabled', async () => {
    // The counterpart of the first case above, and the half HEDGE-180 leaned
    // on: with ACL hints present, the default is to navigate the document to
    // /login. Callers that only want the picker to fail (not the page) opt out
    // with this header, so the 403 has to come back as a plain Response and
    // leave no pending `setTimeout` carrying `location.href = '/login…'`.
    const response = createMockResponse(403, {
      error: 'Forbidden',
      requiredFeatures: ['auth.users.list'],
    })
    ;(window as unknown as Record<string, unknown>).__omOriginalFetch = jest.fn(async () => response)

    const result = await apiFetch('/api/auth/users?pageSize=50', {
      headers: {
    ...
    expect(window.location.pathname).toBe('/backend/sales/documents')
  })
