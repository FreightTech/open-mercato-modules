"use client"

import * as React from 'react'
import Link from 'next/link'
import { Mail } from 'lucide-react'
import { useT } from '@open-mercato/shared/lib/i18n/context'
import { cn } from '@open-mercato/shared/lib/utils'
import { IconButton } from '../../primitives-v2/IconButton'
import { NotificationCountBadge } from '../notifications/NotificationCountBadge'
import { useMessagesPoll } from './useMessagesPoll'

export type MessagesIconProps = {
  className?: string
}

export function MessagesIcon({ className }: MessagesIconProps) {
  const t = useT()
  const { unreadCount, hasNew } = useMessagesPoll()

  const ariaLabel = unreadCount > 0
    ? t('messages.badge.unread', '{count} unread messages', { count: unreadCount })
    : t('messages.nav.inbox', 'Messages')

  return (
    <IconButton variant="ghost" size="sm" asChild aria-label={ariaLabel} className={cn('relative', className)}>
      <Link href="/backend/messages" aria-label={ariaLabel}>
        <Mail className={cn('h-4 w-4 text-topbar-v2-icon', hasNew && 'animate-pulse')} />
        <NotificationCountBadge count={unreadCount} />
      </Link>
    </IconButton>
  )
}
