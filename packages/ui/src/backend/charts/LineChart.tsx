...
export type LineDashStyle = 'solid' | 'ShortDash' | 'Dash' | 'Dot'

/** Map a Highcharts-style dashStyle to a Recharts `strokeDasharray`. `solid` → undefined. */
const DASH_ARRAY: Record<LineDashStyle, string | undefined> = {
  solid: undefined,
  ShortDash: '6 3',
  Dash: '8 5',
  Dot: '2 4',
...
export type LineChartProps = {
  ...
  /**
   * Per-category line dash style (Highcharts-style names mapped to Recharts
   * `strokeDasharray`: `ShortDash`→"6 3", `Dash`→"8 5", `Dot`→"2 4", `solid`→none).
   * Categories absent from the map keep the solid default — e.g. chart 8:
   * Plan=ShortDash / Wykonanie=solid / Predykcja=Dot.
   */
  seriesDash?: Record<string, LineDashStyle>
}
...
export function LineChart({
  ...
  const getLineDash = (category: string): string | undefined => {
    const style = seriesDash?.[category]
    return style ? DASH_ARRAY[style] : undefined
  }
            ...
            {showArea
              ? categories.map((category, idx) => (
                  <Area
                    ...
                    strokeDasharray={getLineDash(category)}
