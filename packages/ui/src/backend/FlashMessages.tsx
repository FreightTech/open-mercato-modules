...
function FlashMessagesInner() {
  ...
  const showFlash = React.useCallback((message: string, type: FlashKind) => {
    ...
    setMsg(message)
    setKind(type)
    // Errors stay until the user dismisses them. A failed save is the one case where the
    // message is the only thing telling the user their work was NOT persisted, and a
    // three-second toast leaves a screen that looks exactly like a successful save.
