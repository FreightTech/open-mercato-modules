import type { UseNotificationsPollResult } from '@freighttech/ui/backend/notifications/useNotificationsPoll'
...
jest.mock('@freighttech/ui/backend/notifications/useNotificationsPoll', () => ({
  useNotificationsPoll: jest.fn(() => pollResult),
}))

jest.mock('@freighttech/ui/backend/notifications/useNotificationsSse', () => ({
...
describe('useNotifications strategy', () => {
  ...
  it('uses SSE strategy when EventSource is available', async () => {
    ...
    const { useNotifications } = await import('@freighttech/ui/backend/notifications/useNotifications')
