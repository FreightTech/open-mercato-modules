...
describe('AppShell', () => {
  ...
  it('resets breadcrumb to server-provided values when pathname changes', async () => {
    ...
    // Select by accessible name, not by className. The previous selector looked
    // for a `text-sm` class that AppShell stopped emitting when the breadcrumb was
    // restyled, so this helper silently returned '' and the assertion below could
    // never pass — the test was failing on its own selector, not on the behaviour.
    const getBreadcrumbText = () =>
      screen.queryByRole('navigation', { name: 'Breadcrumb' })?.textContent ?? ''
  ...
  it('keeps settings parent item active on descendant routes outside explicit child list', async () => {
    ...
    await waitFor(() => {
      // The v2 Sidebar marks active rows via `aria-current="page"` (the
      // pill background class is a v2 detail and not asserted here).
  ...
  it('refreshes sidebar nav from adminNavApi when om:refresh-sidebar event fires', async () => {
    ...
    try {
      ...
      // Initially no nav groups (empty groups prop)
      expect(screen.queryByText('Users List')).not.toBeInTheDocument()

      // Trigger sidebar refresh via the custom event the component listens on
