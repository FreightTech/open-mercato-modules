import type { UseProgressPollResult } from '@freighttech/ui/backend/progress/useProgressPoll'
...
jest.mock('@freighttech/ui/backend/progress/useProgressPoll', () => ({
  useProgressPoll: jest.fn(() => pollResult),
}))

jest.mock('@freighttech/ui/backend/progress/useProgressSse', () => ({
...
describe('useProgress strategy', () => {
  ...
  it('uses SSE strategy when EventSource is available', async () => {
    ...
    const { useProgress } = await import('@freighttech/ui/backend/progress/useProgress')
